...
<dl id="Dialogue_' . $ID . '">' . "\n";
			...
			foreach($Ligne as $Auteur=>$Phrase)
			{
				...
				{//Ajoute une référence
					$Reference = '<span class="Reference floatR">' . str_replace('...', '&hellip;', preg_replace('#(é|É)pisode ([1-9]?[0-9])#i','<a href="Episode-$2">$0</a>',preg_replace('#"(.+)"#isU','«&nbsp;<strong>$1</strong>&nbsp;»',preg_replace('#\*(.+)\*#isU','<em>$1</em>',preg_replace('#«\s?(.+)\s?»#isU','«&nbsp;$1&nbsp;»',substr($Phrase,strpos($Phrase,"REF:")+4)))))) . '</span>';
